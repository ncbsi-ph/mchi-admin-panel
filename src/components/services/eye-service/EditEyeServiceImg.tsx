...
import { editEyeServiceImg, uploadImage } from '../../../api';
import ServiceImgFormModal from '../ServiceImgFormModal';

const EditEyeServiceImg = ({ data }: { data: ServicesImg }) => {
  ...
  const editImgMutation = useMutation({
    mutationFn: async () => {
      try {
        ...
        const response = await editEyeServiceImg(payload, data.id, token);
...
export default EditEyeServiceImg;
