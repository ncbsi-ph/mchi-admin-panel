...
import { editRadiologyServiceImg, uploadImage } from '../../../api';
import ServiceImgFormModal from '../ServiceImgFormModal';

const EditRadiologyServiceImg = ({ data }: { data: ServicesImg }) => {
  ...
  const editImgMutation = useMutation({
    mutationFn: async () => {
      try {
        ...
        const response = await editRadiologyServiceImg(payload, data.id, token);
...
export default EditRadiologyServiceImg;
