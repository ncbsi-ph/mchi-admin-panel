...
import { editSpecialServiceImg, uploadImage } from '../../../api';
import ServiceImgFormModal from '../ServiceImgFormModal';

const EditSpecialServiceImg = ({ data }: { data: ServicesImg }) => {
  ...
  const editImgMutation = useMutation({
    mutationFn: async () => {
      try {
        ...
        const response = await editSpecialServiceImg(payload, data.id, token);
...
export default EditSpecialServiceImg;
